...
public class VehicleRentalStatisticsController {
    ...
    public ResponseEntity<?> statisticByLocationOrVehicleTypeDate(
            @RequestParam(required = false, value = "location") String location,
            @RequestParam(required = false, value = "vehicle_type") String vehicleTypeName,
            @RequestParam("start_date") String startDate,
            @RequestParam("end_date") String endDate) {
        return ResponseEntity.status(HttpStatus.OK).body(
                vehicleRentalStatisticService.statisticFromLocationOrVehicleTypeByDate(location, vehicleTypeName, startDate, endDate));
    }

    @GetMapping("/statistic-from-location-or-vehicleType-by-month-and-year")
    public ResponseEntity<?> statisticByLocationOrVehicleTypeMonthAndYear(
            @RequestParam(required = false, value = "location") String location,
            @RequestParam(required = false, value = "vehicle_type") String vehicleTypeName,
            @RequestParam("month") int month,
            @RequestParam("year") int year) {
        return ResponseEntity.status(HttpStatus.OK).body(
                vehicleRentalStatisticService.statisticFromLocationOrVehicleTypeByMonthAndYear(location, vehicleTypeName, month, year));
    }

    @GetMapping("/statistic-from-location-or-vehicleType-by-year")
    public ResponseEntity<?> statisticByLocationOrVehicleTypeYear(
            @RequestParam(required = false, value = "location") String location,
            @RequestParam(required = false, value = "vehicle_type") String vehicleTypeName,
            @RequestParam("year") List<Integer> years) throws ApplicationException {
        return ResponseEntity.status(HttpStatus.OK).body(
                vehicleRentalStatisticService.statisticFromLocationOrVehicleTypeByYear(location, vehicleTypeName, years));
    }
